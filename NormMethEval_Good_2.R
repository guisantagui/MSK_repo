setwd("/Users/santamag/Desktop/GUILLEM/wrkng_dirs/NormMet_Good")
...
########################################################################################################################################################
#
# Read the data and prepare the format: build a Re-curated version of the metabolite table using dictionary.
#
########################################################################################################################################################

#Load Names
sampleNames <- read_excel("/Users/santamag/Desktop/GUILLEM/metabolomics_jx/curated_peaks/sampleName1.xlsx")
colnames(sampleNames) <- paste(sampleNames[1, ], sampleNames[2, ], sep = '_')

#Build hybrid matrix

load("/Users/santamag/Desktop/GUILLEM/wrkng_dirs_newData/normMetAnal_ND/metabolitePeaks.RData")
load("/Users/santamag/Desktop/GUILLEM/wrkng_dirs/metabolitePeaks_preCur.RData")
load("/Users/santamag/Desktop/GUILLEM/wrkng_dirs_newData/dictionary_NewOld_dataSets/dictionary.RData")



metPeaksConsens <- metabolitePeaks_preCur[!metabolitePeaks_preCur$`Compound Name` %in% dictionary$`Old Data Names`[dictionary$State == "Redundant"], ]
metPeaksConsens <- metPeaksConsens[, -c(2:4, ncol(metPeaksConsens))]
for(i in 1:nrow(metPeaksConsens)){
        if(ifelse(dictionary$State[which(dictionary$`Old Data Names` == metPeaksConsens$`Compound Name`[i])] %in% c("Removed", "Renamed", "Misspelled", "Same", "Renamed, and duplicated removed"), T, F)){
                metPeaksConsens$`Compound Name`[i] <- dictionary$Consensus[which(dictionary$`Old Data Names` == metPeaksConsens$`Compound Name`[i])]
        }
}

newAdded <- metabolitePeaks[metabolitePeaks$`Compound Name` %in% dictionary$`New Data Names`[dictionary$State == "Added"], ]
newAdded <- newAdded[, -ncol(newAdded)]
colnames(newAdded) <- colnames(metPeaksConsens)
metPeaksConsens <- rbind.data.frame(metPeaksConsens, newAdded)

metabolitePeaks <- metPeaksConsens
compNames <- metabolitePeaks$`Compound Name`
save(compNames, file = "compNames.RData")
...
metabolomicsPackageFormat <- data.frame(t(metabolitePeaks))
colnames(metabolomicsPackageFormat) <- compNames
...
metabolomicsPackageFormat <- cbind.data.frame("Group" = metabolomicsPackageFormat$Group, 
                                              apply(metabolomicsPackageFormat[, 2:ncol(metabolomicsPackageFormat)], 
                                                    2, as.numeric))
par(mar = c(1, 1, 1, 1))
...
p + geom_col() + 
        ...
        ggsave("/Users/santamag/Desktop/GUILLEM/wrkng_dirs/plots/meanPeakAreaRaw.jpg")
...
featuredataMet <- metabolomicsPackageFormat[, 2:ncol(metabolomicsPackageFormat)]
...
save(missingMet, file = "missingMet.RData")
...
sink("ranksAllStrainsMeths.csv", type = "output")
...
ggsave("meanRanksTstatVert.pdf")
ggsave("meanRanksaTstatVert.tiff", device = "tiff")   
...
write.csv(norm_ccmn_normalizemets$featuredata, file = "normalized_ccmn_good.csv")
...
HeatMap(norm_ccmn_normalizemets$featuredata, 
        scale = 'column',
        ColSideColors = Cols, 
        colramp = bluered(75),
        ...
        aggmethod = "ward.D",
        cexRow = 0.65)
HeatMap(norm_nomis_normalizemets$featuredata, 
        scale = 'column',
        ColSideColors = Cols, 
        colramp = bluered(75),
        distmethod = "euclidean",
        aggmethod = "ward.D",
        cexRow = 0.65)
HeatMap(norm_mean_normalizemets$featuredata, 
        scale = 'column',
        ColSideColors = Cols, 
        colramp = bluered(75),
        distmethod = "euclidean",
        aggmethod = "ward.D",
        cexRow = 0.65)
HeatMap(norm_ruvrandclust_normalizemets$featuredata, 
        scale = 'column',
        ColSideColors = Cols, 
        colramp = bluered(75),
        distmethod = "euclidean",
