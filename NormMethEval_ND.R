setwd("/Users/santamag/Desktop/GUILLEM/wrkng_dirs_newData/normMetAnal_ND")
...
mets <-read_xlsx("/Users/santamag/Desktop/GUILLEM/wrkng_dirs_newData/normMetAnal_ND/metaboliteTable.xlsx")
...
posMat <- cbind(posMetNames, posMat)
...
negMat <- cbind(negMetNames, negMat)
...
save(compoundNames, file = "compoundNames.RData")

batches <- rbind(posBatches, negBatches)
batches <- apply(batches, 2, unique)

#Save data as .txt
write.table(metabolitePeaks, 
            sep = "\t", 
            file = "/Users/santamag/Desktop/GUILLEM/wrkng_dirs_newData/normMetAnal_ND/curatePeaks_ND.txt", 
            row.names = F)

# As with the curated data we end up with very few non-changing metabolites the pre-curated data is loaded in order to use its nonchanging mets too
load(file = "../../wrkng_dirs/metabolitePeaks_preCur.RData")
metabolitePeaks_preCur
...
metabolomicsPackageFormat <- data.frame(t(metabolitePeaks[, -1]))
...
write.csv(metabolomicsPackageFormat, file = "metabolomicsPackageFormat.csv")
...
p + geom_col() + 
        ...
        ggsave("/Users/santamag/Desktop/GUILLEM/wrkng_dirs_newData/normMetAnal_ND/meanPeakAreaRaw.jpg")
...
# The curated dataset is supposed to have some metabolites whose identity was ambiguous removed, so being that all the metabolite names in 
# in the curated dataset should appear in the pre-curated dataset

metabolitePeaks$`Compound Name`[!metabolitePeaks$`Compound Name` %in% metabolitePeaks_preCur$`Compound Name`] #--> those don't appear

metPeaksOld_notInNew <- metabolitePeaks_preCur[!metabolitePeaks_preCur$`Compound Name` %in% metabolitePeaks$`Compound Name`, ]
write.csv(metPeaksOld_notInNew, file = "metPeaksOld_notInNew.csv")
...
# Deoxyribose 2 ---------------------> Doesn't appear Maybe dihydroxy-isovalerate 1?
# Galactono-1,4-lactone -------------> Appears, but is mispelled
# gliceraldehyde3p ------------------> Doesn't appear New in the new file
# hexose diphosphate ----------------> Doesn't appear. The closest one is hexose phosphates. Let's check the numbers New in the new file.
...
# Methylcitrate 1 -------------------> Doesn't appear Homocitrate?
...
metabolitePeaks_preCur$`Compound Name`

# Add nonchanging mets columns we don't have in curated data from pre curated data



metabolomicsPackageFormat <- cbind.data.frame(metabolomicsPackageFormat,
                                              t(metabolitePeaks_preCur[which(metabolitePeaks_preCur$`Compound Name` == "-Acetyl-L-glutamate 5-semialdehyde"), 
                                                                       5:(ncol(metabolitePeaks_preCur) - 1)]))
metabolomicsPackageFormat <- cbind.data.frame(metabolomicsPackageFormat,
                                              t(metabolitePeaks_preCur[which(metabolitePeaks_preCur$`Compound Name` == "Tyramine"), 
                                                                       5:(ncol(metabolitePeaks_preCur) - 1)]))
metabolomicsPackageFormat <- cbind.data.frame(metabolomicsPackageFormat,
                                              t(metabolitePeaks_preCur[which(metabolitePeaks_preCur$`Compound Name` == "Acetolactate"), 
                                                                       5:(ncol(metabolitePeaks_preCur) - 1)]))
metabolomicsPackageFormat <- cbind.data.frame(metabolomicsPackageFormat,
                                              t(metabolitePeaks_preCur[which(metabolitePeaks_preCur$`Compound Name` == "Methylglyoxal"), 
                                                                       5:(ncol(metabolitePeaks_preCur) - 1)]))

colnames(metabolomicsPackageFormat)[(78-3):78] <- c("-Acetyl-L-glutamate 5-semialdehyde", 
                                                    "Tyramine",
                                                    "Acetolactate",
                                                    "Methylglyoxal")


# In the analysis I did past year due to a function removing 2 metabolites from the dataset, the nonchanging metabolite list was shifted, taking these 9 as nonchanging mets. 
View(missingMet$featuredata[, c("X.Acetyl.L.glutamate.5.semialdehyde", 
                                "X4.Aminobutanoate.3",
                                "Acetyl.2.6.diaminopimelate", #--> Is in new data 9
                                "glutamate", # --> Is in new data 55
                                "Glycerate", # --> Is in new data 58
                                "Leucine", # --> Is in new data 32
                                "methylmaleate.1", # --> Is in new data 68
                                "S.Adenosylmethionine", # --> Is in new data 40
                                "Tyrosine")]) # --> Is in new data 44

# Let's try to normalize the new data using those mets as nonchanging-mets.
...
ggsave("const_metaboliteds.pdf",width = 7, height = 14)
...
norm_ccmn_normalizemets2 <- NormQcmets(missingMet$featuredata, method = "ccmn", 
                                       qcmets = which(IS2 == 1), factors = sampledataMet$Group,
                                       saveoutput = T)
metvarCcmnNormalizeMets2 <- metvar(norm_ccmn_normalizemets2$featuredata, log.data[, 1])
neibCcmnNormalizeMets2 <- neibcheck(norm_ccmn_normalizemets2$featuredata)
...
ggsave("normmethod_dist_normalizeMets.pdf")
...
ggsave("pvaluehist_pre.jpeg", device = "jpeg")
...
ggsave("pvaluehist_nomis.jpeg", device = "jpeg")
...
ggsave("pvaluehist_ruvrandclust.jpeg", device = "jpeg")
...
RlaPlots(featuredata = norm_ccmn_normalizemets2$featuredata, 
         groupdata = sampledataMet$Group, type = "ag",
         interactiveplot = F, interactiveonly = F, 
         cols = rainbow(28), minoutlier = 0.5, las = 3,
         plotname = "Across-Group, CCMN2",
         saveplot = T, savetype = "tiff", showlegend = T,
         ylim = c(-2, 2))
...
write.csv(norm_ccmn_normalizemets2$featuredata, file = "normalized_ccmn2_ND.csv")
